...
#include "myV775.h"
#include "myV862AC.h"
#include "sigHandler.h" // Provides setMess()
...
//v1742 DRS4(0x0032000, "/dev/vmedrv32d32", "/home/dreamtest/working/drs4command.list");
v1742 DRS4(0x0032000, "/dev/vmedrv32d32", "/etc/wavedump/WaveDumpConfig.txt");
...
double tref(0), t_istrigger(1), t_rdoarm(0), t_readev(0), t_mtrigger(0), t_trghandler(0), t_gettek(0), t_usleep(0), t_schedy(0);
uint64_t n_loop(0);
...
int myReadEvent(bool isPhysEv){
  ...
  // Check DRS4 chips are not busy
  //while (DRS4.busy()) { usleep(100); }
  size= DRS4.readSingleEvent(&buf[*bufsize]);

  uint32_t drs4_size = size;
...
/****************************************/
{
  ...
  cout << tprint() << " Run " << runnr << " starting, going to acquire " << maxtotevts << " events" << endl;
  ...
  cout << tprint() << " Waiting for a spill ... " << flush;
  ...
  while(1){
    ...
    char dbg_info[265];
    sprintf(dbg_info,"begin of while(1), events: %d, pedevents: %d, or: %d",
		events, pedevents, events || pedevents);
    setMess(dbg_info);

    timestr = time(NULL);
    if(isTriggerPresent(&trigmask)){
      setMess("just got a trigger, calling handler");
      ...
      setMess("setting/clearing pedestal veto");
      timestr = time(NULL);
      if(phys_enabled){
	if(trigmask & 0x2){
	  setPedTriggerVeto();
    ...
    if(!isInSpill() || 
      ...
      setMess("spill or osc memory is over: setTriggerVeto()");
      setTriggerVeto();
      setMess("spill or osc memory is over: setPedTriggerVeto()");
      setPedTriggerVeto();
      usleep(200);
      spills++;
      ...
      setMess("Get spill number");
      myFIFOUpdateSpillNr();
        
      if(isTriggerPresent(&trigmask)){

        timestr = time(NULL);
	setMess("handle trigger");
	myTriggerHandler(trigmask,maxpedevts,
			 &tv0,&tsum,&bsum,
			 &ped_enabled);
	// unlockTrigger();
      ...
      if (samples != nev) {
        cout << tprint() << " 1. Oscilloscope samples (" << samples << ") != from VME events ("
	...
	cout << tprint() << " Oscilloscope data update failed" << endl;
	abort_run=true;
      } else osc.start();
      rlat = time(NULL)-rlat;
      cout << tprint() << " - FIFO Update done for " << samples
           << " evt.s in " << rlat << " sec" << endl;
#endif
      
      setMess("get FIFO occupancy");
      double occ=myFIFOOccupancy();
      setMess("get ped FIFO occupancy");
      ...
      if (events || pedevents) {
	setMess("if there are events print spill info");
	...
	cout << spills << " - Total Events " << totevents
      ...
      while(!isInSpill() && !abort_run){
	sprintf(dbg_info,"waiting for new spill, events: %d, pedevents: %d, or: %d",
		events, pedevents, events || pedevents);
	setMess(dbg_info);
  ...
  uint32_t samples = GetTekData(runnr,spills,nev,maxoscevts);
  if (samples != nev) {
    cout << tprint() << " 2. Oscilloscope samples " << samples << " differ from VME: "
