//$Id: doSwapFADC7-8.cpp,v 1.1 2006/10/31 15:08:50 cvsdream Exp $
#include <iostream>
#include <stdio.h>
...
int main(int argc, char * argv[]){
  ...
  while(fread(&word,sizeof(unsigned int),1,datafile)==1){
    ...
    unsigned int evsz = head.evsiz/sizeof(unsigned int);
    unsigned int * fadcp7 = buf+evsz-111;
    unsigned int * fadcp8 = buf+evsz-101;
    for (int i=0; i<8; i++) {
      unsigned int sw = *(fadcp7+i);
      *(fadcp7+i) = *(fadcp8+i);
      *(fadcp8+i) = sw;
    }
