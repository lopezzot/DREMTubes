//$Id: myFIFO.cpp,v 1.6 2009/07/21 09:14:45 dreamdaq Exp $
#include <iostream>
#include <fcntl.h>
#include <unistd.h>
#include <sys/ipc.h>
...
myFIFO::myFIFO(key_t base_key, myFIFO::userType aType){
  ...
  int my_semid = semget(SEMKEY+base_key,2,IPC_CREAT|S_IRUSR|S_IWUSR);
  ...
  //Get the data shm
  int dataShmId = shmget(DATAKEY+base_key,
...
std::cout << "internalData->datashmid " << hex << internalData->datashmid <<
" internalData->semid " << internalData->semid << dec << endl;
std::cout << hex << " base_key PHYS_BASE_KEY PED_BASE_KEY " << base_key << " " << PHYS_BASE_KEY << " " << PED_BASE_KEY << dec << endl;
std::cout << hex << " m_size " << m_size << dec << endl;
    ...
    for(register unsigned int i=0;i<MAXSM;i++){
  ...
  if(type==myFIFO::READER){
    internalData->startRD=(unsigned int*)shmat(internalData->FIFOshmid,
						NULL,0);

    printf("READER Buffer start %p end %p\n",internalData->startRD, internalData->startRD + m_size);

    if(internalData->startRD==(unsigned int *)-1){
    ...
    internalData->startWR=(unsigned int*)shmat(internalData->FIFOshmid,
					       NULL,0);
    
    printf("WRITER Buffer start %p end %p\n",
           internalData->startWR, internalData->startWR + m_size);

    if(internalData->startWR==(unsigned int *)-1){
      cout<< "Cannot attach FIFO shm" << endl;
      valid=false;
      semop(my_semid,&ops[2],1);
      return;
    ...
    internalData->startSM[internalData->nsampler]=
      (unsigned int*)shmat(internalData->FIFOshmid,NULL,0);
    
    printf("SAMPLER Buffer start %p end %p\n",
           internalData->startSM[internalData->nsampler],
           internalData->startSM[internalData->nsampler]+m_size);

    if(internalData->startSM[internalData->nsampler]==(unsigned int *)-1){
      cout<< "Cannot attach FIFO shm" << endl;
      valid=false;
      semop(my_semid,&ops[2],1);
      return;
...
myFIFO::~myFIFO(){
  ...
  int mysemid=internalData->semid;
  int dataid=internalData->datashmid;
...
void myFIFO::restore(){
  size_t sz = sizeof(struct myFIFOData);
  unsigned int sn = internalData->spillnr;
