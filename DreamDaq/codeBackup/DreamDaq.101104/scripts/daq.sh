...
usage()
 {
  echo  "usage: daq [clean|stop|vme|osc|all] [# of evt.s]" 
 }

hname=`hostname`
if [[ "$hname" != "pcdream" ]]; then
  echo "wrong host: \"$hname\" --> please run \"daq\" from pcdream"
  usage
fi

if [ "$1" == "clean" ] ; then
  echo "ssh dreamtest@pcdreamdaq dreamProc.sh clean"
  ssh dreamtest@pcdreamdaq "dreamProc.sh clean" &
...
if [ "$1" == "stop" ] ; then
  echo "ssh dreamtest@pcdreamdaq dreamProc.sh stop"
  ssh dreamtest@pcdreamdaq "dreamProc.sh stop"
  sleep 3
  exit 0
fi
...
#if [ "$opt" != "vme" ] && [ "$opt" != "osc" ] && [ "$opt" != "all" ]; then
  echo "wrong call: $*"
  usage
  exit 1
fi
...
printsummaries ()
 {
  ...
  if [ -s "$lgfile" ] ; then
    ...
    if [ "$string" != "" ] ; then
      ...
      echo -e "\n\n*** ERROR : Missing lines in sampler output (DAQ stopped in advance??) ***\n\n"
    fi
  else
...
dreamproc()
 {
  ...
  gnome-terminal --geometry=100x18+$xoff+$yoff --zoom=0.8 -e "ssh dreamtest@$1 \"dreamProc.sh $2 $3\" ; echo \"Exiting ....\" ; sleep 3" &
  yoff=`expr $yoff + 290`
 }
...
echo "---> starting readout on pcdreamdaq"
dreamproc pcdreamdaq $opt $nevts
sleep 2
...
echo "---> starting data writer on pcdreamdaq"
dreamproc pcdreamdaq data
sleep 2

echo "---> starting pedestal writer on pcdreamdaq"
dreamproc pcdreamdaq ped
sleep 2

#echo "---> starting sampler on pcdreamdaq"
#dreamproc pcdreamdaq mon
#sleep 2
...
#takeCamScreenshots.sh 30 &
echo "Cam screenshot disabled"
...
sleep 10 # Start the sampler later
echo "---> starting sampler on pcdreamdaq"
dreamproc pcdreamdaq mon
sleep 2
...
# Owing that this bash script is executed with the nasty '-f' parameter
# (why the author used this fuck... parameter? WHY??!!!), the shell do not
# expand regular expressions and therefore 'cp *.dat' fails.
# So one is compelled to use a dirty trick with 'find' 
echo "Backing up data"
#cp -v /mnt/working/data/datafile_run${runnr}_0.dat /mnt/wd/working_bak/data 
find /mnt/working/data -maxdepth 1 -name "datafile_run${runnr}*.dat" -exec cp -v {} /mnt/wd/working_bak/data \;
find /mnt/working/data -maxdepth 1 -name "pedestal_run${runnr}*.dat" -exec cp -v {} /mnt/wd/working_bak/pedestal \;
echo "Ok, files backed up in /mnt/wd/working_bak/data"
