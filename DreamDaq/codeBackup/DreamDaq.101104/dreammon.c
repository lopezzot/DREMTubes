...
int dreammon_init(char ** argv, unsigned int run_nr, bool drs,int drs_setup,bool phys_h)
{
  ...
  for(int i_osc_ch = 0; i_osc_ch <N_CH_OSC; i_osc_ch++ ){
    ...
    //h_osc_int[i_osc_ch] = new TH1F(histo, histo,  RANGE_OSC*2, MIN_RANGE_OSC*10,MAX_RANGE_OSC*10);
    h_osc_int[i_osc_ch] = new TH1F(histo, histo,  RANGE_OSC*2, MIN_RANGE_OSC*1064, MAX_RANGE_OSC*1064);
    sprintf(histo,"OSC_ch%d_integral_ped_sub",i_osc_ch);
    h_osc_int_s[i_osc_ch] = new TH1F(histo, histo,  RANGE_OSC*2, -MIN_RANGE_OSC*1064, 0);
...
/*   h_osc_map = new TH2F(histo, histo, N_CH_OSC, 0, N_CH_OSC, 1024,-10240,10240); */
  ...
  for(int i_drs_ch = 0; i_drs_ch <N_CH_DRS; i_drs_ch++ )
  {
    switch(i_drs_ch)
    {
      case  0: sprintf(thename,"DRS_PWO_C1"); break;
      case  1: sprintf(thename,"DRS_PWO_C2"); break;
      case  2: sprintf(thename,"DRS_PWO_C3"); break;
      case  3: sprintf(thename,"DRS_PWO_C4"); break;
      case  4: sprintf(thename,"DRS_PWO_C5"); break;
      case  5: sprintf(thename,"DRS_PWO_C8"); break;
      case  6: sprintf(thename,"DRS_PWO_C9"); break;
      case  7: sprintf(thename,"DRS_CMS");    break;
      case  9: sprintf(thename,"DRS_PWO_S1"); break;
      case 10: sprintf(thename,"DRS_PWO_S2"); break;
      case 11: sprintf(thename,"DRS_PWO_S3"); break;
      case 12: sprintf(thename,"DRS_PWO_S4"); break;
      case 13: sprintf(thename,"DRS_PWO_S5"); break;
      case 14: sprintf(thename,"DRS_PWO_S8"); break;
      case 15: sprintf(thename,"DRS_PWO_S9"); break;
      default: sprintf(thename,"DRS_ch%d", i_drs_ch); break;
    }
    ...
    for(int i_ev = 0; i_ev <N_1EV_H; i_ev++ ){
      sprintf(histo,"%s_ev%d",thename,i_ev*100);
...
*/
    ...
    h_drs_int_s[i_drs_ch] = new TH1F(histo, histo,  RANGE_DRS*2,
  ...
  h_drs_int_c_sum   = new TH1F(n.c_str(), n.c_str(),  RANGE_DRS*2, -MAX_RANGE_DRS*1000,MAX_RANGE_DRS*1000);
  n.assign("DRS_PWO_S_integral_sum");
  h_drs_int_s_sum   = new TH1F(n.c_str(), n.c_str(),  RANGE_DRS*2, -MAX_RANGE_DRS*1000,MAX_RANGE_DRS*1000);
  n.assign("DRS_PWO_C_integral_sum-pedsubtr");
  h_drs_int_c_sum_p = new TH1F(n.c_str(), n.c_str(),  RANGE_DRS*2, -MAX_RANGE_DRS*1000,MAX_RANGE_DRS*1000);
  n.assign("DRS_PWO_S_integral_sum_pedsubtr");
  h_drs_int_s_sum_p = new TH1F(n.c_str(), n.c_str(),  RANGE_DRS*2, -MAX_RANGE_DRS*1000,MAX_RANGE_DRS*1000);

  h_bungabunga = new TH1F("PedCh10", "PedCh10", 100, 3200, 3600);
...
#define N_PED_SAMPLES_OSC 40
    for (unsigned int j=0; j<tekoscData.index; j++) 
    {
      ...
      integral[ch] += data;
      if(nh<N_PED_SAMPLES_OSC) ped[ch] += data; //accumulate data for first N_PED_SAMPLES 
    }

    for(int i_ch=0; i_ch<N_CH_OSC; i_ch++ )
    {
      float pede = ped[i_ch]/N_PED_SAMPLES_OSC;
      ...
      h_osc_int_s[i_ch]->Fill(integral[i_ch]-pede*N_CELLS_OSC);
      h_osc_int_vs_event[i_ch]->Fill(evt,integral[i_ch]);
      //if(evt < 10)
      //  printf("BungaBunga: %4d ped=%6.1f pedt=%6.1f int=%9d ints=%8.1f  \n", 
      //   i_ch, pede, pede*N_CELLS_OSC, integral[i_ch], integral[i_ch]-pede*N_CELLS_OSC );
    }
  }
  ...
  CAEN_DGTZ_V1742_GROUP_t *g0_raw, *g1_raw;
  CAEN_DGTZ_V1742_GROUP_CORRECTED_t *g0, *g1, g0_, g1_;
  //I know, I should use malloc, but I hate to manage my own memory...
  g0 = &g0_;
  g1 = &g1_;
  uint32_t *ig0, *ig1; 
  ...
  g0_raw = &(EventV1742->Group[0]);
  g1_raw = &(EventV1742->Group[1]);
  ...
  V1742UnpackEventGroup(ig0, g0_raw); 
  ig1 = &addr[3466]; /* 4 (header) + 4 (DRS header) + 1 (ev. desc) + 0xC00 (channels 1-7) + 0x180 (trigger) + 1 (TTT) --> 3466 */
  V1742UnpackEventGroup(ig1, g1_raw); 
  
  //apply cell by cell corrections
  DataCorrection(g0_raw,g0,0);
  DataCorrection(g1_raw,g1,1);
...
/*       if(evt<2){ */
/* 	cout << i_ch << ", " << cell << ") " */
/* 	     << g0_raw->Data[i_ch][cell] << " -> " */
/* 	     << g0->Data[i_ch][cell] << endl; */
/*       } */
...
/* 	 << integral[i_ch]-ped[i_ch]*1024 << endl; */
    ...
    if(i_ch==10)
	h_bungabunga->Fill(ped[i_ch]);
    //printf("CH: %4d Ped: %9.1f Integral: %9d Diff: %9.1f\n", i_ch, ped[i_ch]*1024, integral[i_ch], ped[i_ch]*1024 -float(integral[i_ch]) );
...
int dreammon_exit(unsigned int i, bool drs,int drs_setup){
  ...
  for(int i_drs_ch = 0; i_drs_ch <N_CH_DRS; i_drs_ch++ ){
    ...
    std::string n = h_drs[i_drs_ch]->GetName();
  ...
  h_drs_int_c_sum->Write();
  h_drs_int_s_sum->Write();
  h_drs_int_c_sum_p->Write();
  h_drs_int_s_sum_p->Write();
  h_bungabunga->Write();
