     ...
     /********************************************/
{
  ...
  if (beginningOfRun) { // Only at the beginning of a new run
    ...
    unsigned int tmp;
    fflush(stdout);
    rc = fread(&tmp, sizeof(unsigned int), 1, rawdatafp);
     ...
     /********************************************/
{
  ...
  while(i<GetSubEventSize()/4)
  {
    if( v775::dataDecodeType(buf[i]) != 0 )
      std::cerr << "V775 problem: invalid data type!" << std::endl;
    tdc->data[hits]   = v775::dataDecodeValue   (buf[i]);
    tdc->channel[hits]= v775::dataDecodeChannel (buf[i]);
    tdc->valid[hits]  = v775::dataDecodeValidity(buf[i]);
  //    tdc->edge[hits]=(buf[i]&0x10000)>>16;
/*     printf("[TDC755] hit = %2d | ch = %2d | counts = %5d | raw = %8X\n", */
/*            hits, tdc->channel[hits], tdc->data[hits], buf[i] ); */
    i++;
    hits++;
  }
  tdc->index = hits;
...
/********************************************/
int DecodeV862(const unsigned int *buf, myADCN *adcn)
     /********************************************/
{
