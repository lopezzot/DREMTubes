...
//    l2->setGeometry(280,291,31,16);
    ...
    connect(ui->actionStart,SIGNAL(triggered()),this,SLOT(startStopSlot()));
...
void DaqControlInterface::started(){
    ...
    QString prgr = "desyfunct.sh start"+QString(" %1 ").arg(runNumber)+rdo+QString(" %1 %2").arg(eventsNumber).arg(phyToPed);
    qWarning()<<prgr;

    QProcess qp;
    qp.execute(prgr);
    startTime = QDateTime::currentDateTime();
    qp.startDetached("desyfunct.sh sync");
    lastSpillEvents = 0;
    spillCounted = 0;
    countRate.clear();
    openFile();
...
void DaqControlInterface::stopped(){
    ...
    QString prgr="desyfunct.sh stop";
    qWarning()<<prgr;

    QProcess qp;
    qp.execute(prgr);

    prgr="desyfunct.sh log";
    qWarning()<<prgr;
    ...
    QTimer::singleShot(7000,this,SLOT(unlock()));
  // QTest::qSleep(7000);
//    lockInterface(false);
//    lockInfo(false);
}
...
void DaqControlInterface::cleanUpped(){
    ...
    int ret = msgBox.exec();
    QProcess qx;
    QString prgr="desyfunct.sh clean";
    switch(ret){
...
void DaqControlInterface::parseString(QString str){
    ...
    if (toP.count()<20) return;
    this->spillNumber= toP[0].toLong();
    this->realEventsNumber=toP[7].toLong();
    this->phyNumber=toP[13].toLong();
    this->pedNumber=toP[18].toLong();
    if (this->eventsNumber>0){
...
void DaqControlInterface::saveConfigFile(){
    ...
    else qWarning()<<"ERROR: No way to save config! Target file " << readfile << " not found.";
}
...
void DaqControlInterface::unlock(){
        lockInterface(false);
        lockInfo(false);
