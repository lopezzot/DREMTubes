...
int32_t getNumberOfBits(uint8_t byte) {
	uint32_t count(0);
	for (uint32_t i=0; i<8; i++) if (byte & (1<<i)) count++;
	return count;
}

static int32_t V1742UnpackEventGroup(uint32_t /* group */, uint32_t* datain, CAEN_DGTZ_X742_GROUP_t* dataout) {

    int32_t i, rpnt(0), wpnt(0), size1, size2, trg(0), k;
    uint16_t st_ind(0); 
	/*
	uint32_t freq;
...
int32_t V1742UnpackEventGroup2(uint32_t group, uint32_t* datain, CAEN_DGTZ_X742_GROUP_t* dataout) {
  return V1742UnpackEventGroup(group, datain, dataout);
}
...
int32_t GetNumEvents(char* buffer, uint32_t buffsize, uint32_t* numEvents) {
	...
	do {
		counter++;
		evtSize = *(int32_t*)(buffer+i) & 0x0FFFFFFF;
...
int32_t GetEventPtr(char* buffer, 
	...
	if ((buffer == NULL) || (buffsize < EVENT_HEADER_SIZE)){
	  printf("no buffer of buffsize < %x \n",EVENT_HEADER_SIZE);
	  return -1;
	}
	do {
		counter++;
		...
		if (counter == numEvent) {
			if ((i + (uint32_t) evtSize) <= buffsize) {
				*EventPtr =(buffer+i);
			  printf("bsize: %d i: %d i+esize: %d\n",buffsize, i, (i + (uint32_t) evtSize));
...
int32_t X742_DecodeEvent(char* evtPtr, void** Evt) {
	...
	evtSize = *(int32_t*)evtPtr & 0x0FFFFFFF;
	chanMask = *(int32_t*)(evtPtr+4) & 0x0000000F;
