...
#define MAX_GR  8			/* max number of groups */
...
typedef struct {
	int32_t Enable;				// Enable
	int32_t Level;				// Level
	int32_t StatusId;			// Status id
	int32_t EventNumber;			// event_number
	CAEN_DGTZ_IRQMode_t Mode;		// Interrupt mode
	uint8_t	Mask;				// VME Irq wait mask
	int32_t Timeout;			// wait timeout (msec)
} IntConfig_t;
...
typedef struct WaveDumpConfig_t {
    ...
    CAEN_DGTZ_AcqMode_t AcquisitionMode;
    int32_t NumEvents;
    int32_t RecordLength;
    int32_t PostTrigger;
    uint16_t ChannelEnableMask;
...
typedef struct WaveDumpRun_t {
    ...
    int32_t NchPerGroup;
    int32_t Ngroup;
    int32_t ContinuousTrigger;
    int32_t ContinuousWrite;
    int32_t SingleWrite;
    int32_t ContinuousPlot;
    int32_t SinglePlot;
    int32_t GroupPlotIndex;
    int32_t ChannelPlotMask;
    FILE *fout[MAX_GR][MAX_CH];
} WaveDumpRun_t;
