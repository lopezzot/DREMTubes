...
static int32_t read_flash_page(int32_t handle, uint8_t gr, int8_t* page, uint32_t pagenum)
{ 
  uint32_t flash_addr;
  uint16_t	dd;
  uint32_t i,tmp[528];
  ...
  if ((ret = CAENComm_MultiRead32(handle,fl_a,528,tmp,err)) != CAENComm_Success) 
    return -1;
  for (i=0; i<528; i+=2) page[(int32_t)(i/2)] = (int8_t)tmp[i];
  // disable flash (NCS = 1)
...
int32_t LoadCorrectionTables(int32_t handle, DataCorrection_t* Table, uint8_t group, uint32_t frequency) {
  ...
  for (n=0;n<MAX_X742_CHANNELS+1;n++) {
	...
	if (p[j-start] != 0x7f) {
	  ...
	  int16_t cel = (int16_t)((((uint8_t)(p[endidx+1])) << 0x08) |((uint8_t) p[endidx]));
...
void ApplyDataCorrection(DataCorrection_t* CTable, CAEN_DGTZ_DRS4Frequency_t frequency, int32_t CorrectionLevelMask, CAEN_DGTZ_X742_GROUP_t* data) {

  int32_t i, j, size1, trg = 0,k;
  float Time[1024],t0; 
  float Tsamp; 
  float vcorr; 
  uint16_t st_ind=0; 
  float wave_tmp[1024];
  int32_t cellCorrection =CorrectionLevelMask & 0x1;
  int32_t nsampleCorrection = (CorrectionLevelMask & 0x2) >> 1;
  int32_t timeCorrection = (CorrectionLevelMask & 0x4) >> 2;
